...
export default function SolicitudEditModal({ solicitud, onClose, onSaved }: Props) {
  ...
  const handleChange = (key: string, value: any) => setDatos((d) => ({ ...d, [key]: value }));
      ...
      <div className="z-10 w-full max-w-2xl rounded-lg bg-white p-6 shadow-lg">
        ...
        <div className="space-y-3">
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            <button disabled={loading} onClick={handleSubmit} className="rounded bg-brand-600 px-3 py-1 text-sm font-semibold text-white disabled:opacity-60">Guardar</button>
          </div>
        </div>
      </div>
    </div>
