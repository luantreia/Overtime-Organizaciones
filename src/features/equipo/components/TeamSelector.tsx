...
const TeamSelector = () => {
  const { organizaciones, organizacionSeleccionada, seleccionarOrganizacion, loading } = useOrganizacion();

  if (loading) {
    return (
      <div className="h-10 w-40 animate-pulse rounded-lg bg-slate-200" aria-label="Cargando organizaciones" />
    );
  }
