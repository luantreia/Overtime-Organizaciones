      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="text-lg font-semibold text-slate-900">
            {(partido.localNombre || partido.visitanteNombre)
              ? (<>
                  {partido.localNombre || 'Local'} <span className="text-slate-400">vs</span> {partido.visitanteNombre || partido.rival || 'Visitante'}
                </>)
              : (<>vs {partido.rival}</>)}
          </h3>
          ...
          {partido.etapa ? (
            <p className="mt-1 text-[11px] uppercase tracking-wide text-slate-400">Etapa: {String(partido.etapa).replace('_', ' ')}</p>
          ) : null}
        </div>
